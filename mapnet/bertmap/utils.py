...
def biomappings_format_to_bertmap(df:pl.DataFrame):
    ...
    return df.with_columns(
            pl.col("source identifier").map_elements(identifier_to_iri, return_dtype=pl.String).alias("SrcEntity"),
            pl.col("target identifier").map_elements(identifier_to_iri, return_dtype=pl.String).alias("TgtEntity"),
...
def get_known_maps(target_def:dict, source_def:dict,resources:dict, meta:dict, check_biomappings:bool = True, **_):
    """
    saves a file with known mappings parsed from the provided obo file, and also biomappings optionally
    """
    evidence = None
    if check_biomappings:
        forward = load_biomappings_df(target_prefix = target_def['prefix'], source_prefix = source_def['prefix'], undirected = True)
        reverse = load_biomappings_df(source_prefix = target_def['prefix'], target_prefix = source_def['prefix'], undirected = True)
    ...
    save_pth = os.path.join(meta['known_mappings_path'], f"{source_def['prefix']}-{source_def['version']}-{target_def['prefix']}-{target_def['version']}-known_maps.tsv")
    evidence.write_csv(save_pth, separator = '\t', )
    return save_pth
...
def get_resource_file_name(resource_def:dict, resource_path:dict, meta:dict = None, prefix:str = None):
    """
    ...
    resource_name = (
        ...
        if not resource_def["subset"]
            ...
            resource_def['subset_name'], resource_def["prefix"] + ".obo"
        )
    )
    return os.path.join(resource_dir, resource_name)

def get_config(config_path:str, resources:dict, meta:dict, target_def:dict, source_def:dict, resource_path:str, output_path:str = None,global_matching:bool = True, use_auxiliary_mappings:bool = False ):
    """
    ...
    config.output_path = output_path or os.path.join(os.getcwd(), 'output', 'bertmap' ,meta['landscape'],  f'{source_def["prefix"]}-{target_def["prefix"]}')
    ...
    if use_auxiliary_mappings:
        config.auxiliary_ontos = [
              get_resource_file_name(resource_def=resources[x], resource_path = resource_path, meta = meta, prefix = x)
              for x in resources if((x != target_def['prefix']) and (x!= source_def['prefix']))
                ]
...
def load_bertmap(
    ...
    **_
    ...
    config = get_config(config_path = config_path, resource_path=resource_path, resources=resources, meta=meta, target_def = target_def, source_def = source_def, global_matching = global_matching, use_auxiliary_mappings = use_auxiliary_mappings)
    known_map_path = known_map_path or get_known_maps(target_def = target_def, source_def = source_def, resources = resources, meta = meta, check_biomappings= check_biomappings)
    ...
    source_fname = get_resource_file_name(resource_def=source_def, resource_path=resource_path)
    print("loading source onto")
    source_onto = Ontology(source_fname)
    print("loading target onto")
    target_fname = get_resource_file_name(resource_def=target_def, resource_path=resource_path)
